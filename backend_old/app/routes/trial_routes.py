...
def match_trials():
    data = request.json
    print("Received request in /match:", data)
...
def filter_trials():
    ...
    q = Trial.query
    if filters['subtype']: q = q.filter_by(target_subtype=filters['subtype'])
    if filters['phase']: q = q.filter_by(phase=filters['phase'])
    if filters['min_age']: q = q.filter(Trial.max_age >= filters['min_age'])
    if filters['max_age']: q = q.filter(Trial.min_age <= filters['max_age'])
    trials = q.all()
    return jsonify({'count': len(trials), 'trials': [t.to_dict() for t in trials]}), 200
